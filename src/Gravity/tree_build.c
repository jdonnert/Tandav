...
//#ifdef DEBUG
		printf("Top Node %d has %d/%d tree nodes, nodes per part = %g \n",
...
{
//#ifdef DEBUG
	printf("DEBUG: (%d:%d) Sub-Tree Build "
		   "first part=%d npart=%d first node=%d top node=%d\n"
...
//#endif
	...
	const int last = istart + D[tnode_idx].TNode.Npart;
printf("MAIN LOOP %d \n", subtree_start );
	for (int ipart = istart + 1; ipart < last; ipart++) {
...
printf("decline into  node %d nNodes %d \n", node, nNodes);
...
#ifdef DEBUG
	for (int n = subtree_start; n<subtree_start+nNodes; n++) 
		...
		const int tnode_idx, const int top_level)
{
	...
	peanoKey key = Reversed_Peano_Key(px, py, pz) >> (3*top_level);
