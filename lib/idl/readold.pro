...
nmass=0
nbar=h.npart(0)
    IF jj(0) GT -1 THEN nmass=TOTAL(h.npart(jj),/int)
      x=fltarr(3,ntot)
      v=fltarr(3,ntot)
      id=lonarr(ntot)
...
PRO old2new,name
@setconst
   IF NOT(IS_DEF(name)) THEN name='caiusnap_011'
...
;   xH=0.76
;   yhelium = (1. - xH) / (4. * xH)
;   mu = (1. + 4. * yhelium) / (1. + yhelium + xNe)
;
;   unew = FLOAT(u / (g1 / kcgs * mp * e_unit / m_unit * mu))
