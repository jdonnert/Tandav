...
{
	char buf[MAXSTRINGLENGTH], buf1[MAXSTRINGLENGTH],buf2[MAXSTRINGLENGTH],
		 buf3[MAXSTRINGLENGTH];
	bool tagDone[NTags];
	int i, j;
	...
	if (!Task.Rank) {
		...
		while (fgets(buf, MAXSTRINGLENGTH, fd)) {
			...
			case FLOAT:
				*((double *)ParDef[j].addr) = atof(buf2);
				break;
			case STRING:
				strncpy((char *)ParDef[j].addr, buf2, MAXSTRINGLENGTH);
				break;
			case INT:
